        ...
        <header class="page-header">
            ...
            <div class="right-wrapper pull-right">
                <ol class="breadcrumbs">
                    <li>
                        <a class="btn btn-danger" href="{{ url('admin/group/create') }}" style="color:white;">新規グループ登録</a>
                    </li>
                </ol>&nbsp;&nbsp;&nbsp;
            </div>
        </header>
        ...
        <section class="panel">
            ...
            <div class="panel-body">
                ...
                <div class="filter mb-xlg" style="padding: 15px; border: 1px solid #444;">
                    ...
                    <form class="form-horizontal" action="{{ url('admin/group/search/') }}" enctype="multipart/form-data" method="post" id="searchForm">
                        ...
                        <div class="row" style="padding: 0">
                            ...
                            <div class="col-sm-4 mt-md">
                                <div class="row">
                                    <div class="col-sm-12">
                                        <label>グループ名</label>
                                        <input id="g_name" name="g_name" class="form-control" value="{{  $data['g_name'] }}" />
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="row">
                                    <div class="col-sm-12">
                                        ...
                                        <select id="g_status" name="g_status" class="form-control">
                                            <option value=""></option> 
                                            <option value="契約中" @if($data['g_status'] == '契約中') selected @endif>契約中</option> 
                                            <option value="休止" @if($data['g_status'] == '休止') selected @endif>休止</option> 
                                        </select>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="row">
                                    <div class="col-sm-12">
                                        ...
                                        <select id="g_kind" name="g_kind" class="form-control">
                                            <option value=""></option> 
                                            <option value="グループ店" @if($data['g_kind'] == 'グループ店') selected @endif>グループ店</option> 
                                            <option value="代理店" @if($data['g_kind'] == '代理店') selected @endif>代理店</option> 
                                        </select>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="col-sm-4 mt-md">
                                <div class="row">
                                    <div class="col-sm-12">
                                        <label>加盟店ID</label>
                                        <input type="text" id="merchant_id" name="merchant_id" class="form-control" value="{{  $data['merchant_id'] }}" />
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="col-sm-4 mt-md">
                                <div class="row">
                                    <div class="col-sm-12">
                                        <label>店舗ID</label>
                                        <input id="shop_id" name="shop_id" class="form-control" value="{{  $data['shop_id'] }}" />
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="col-sm-4 mt-md">
                                <div class="row">
                                    <div class="col-sm-12">
                                        <label>店舗名</label>
                                        <input id="shop_name" name="shop_name" class="form-control" value="{{  $data['shop_name'] }}" />
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <table class="table table-bordered table-striped text-center" id="group_datatable">
                    ...
                    <tbody>
                        ...
                        @foreach($list as $group)
                            <tr>
                                <td>{{ $index }}</td>
                                <td>{{ substr($group->created_at,0,10) }}</td>
                                <td><a class="link" href="{{ url('admin/group/edit/' . $group->id) }}">{{ $group->name }}</a></td>
                                <td><a class="link" href="{{ url('admin/group/edit/' . $group->id) }}">{{ $group->g_name }}</a></td>
                                <td>
                                    @if($group->g_merchantcount > 0)
                                        <a href="{{ url('admin/group/merchants/' . $group->id) }}" style="color: white;padding: 5px 10px; border-radius: 15px;background-color: orange">{{ $group->g_merchantcount }}</a>
                                    @else
                                        {{ $group->g_merchantcount }}
                                    @endif
                                </td>
                                <td>
                                    @if($group->g_shopcount > 0)
                                        <a href="{{ url('admin/group/shops/' . $group->id) }}" style="color: white;padding: 5px 10px; border-radius: 15px;background-color: orange">{{ $group->g_shopcount }}</a>
                                    @else
                                        {{ $group->g_shopcount }}
                                    @endif
                                </td>
                                ...
                                <td>{{ $group->g_kind }}</td>
                                <td>
                                    <a class="btn btn-danger remove-row" data-plugin-tooltip data-toggle="tooltip" data-placement="top" data-original-title="削除" data-url="{{ url('/admin/group/delete/' .$group->id) }}"><i class="fa fa-trash" style="color: white;"></i></a>
                                </td>
                            </tr>
                            @php( $index++ )
                        @endforeach
...
@section('page_js_vendor')
    ...
    <script src="{{ asset('public/admin/js/views/group.js') }}"></script>
    ...
    <script>
        ...
        $( document ).ready(function() {
            ...
            $('select[name="group_datatable_length"]').on('change', function() {
                ...
                window.location.href = '{{ url('/admin/group/') }}' + '?' + $('#searchForm').serialize();
            });
        });
