        ...
        <section class="panel">
            ...
            <div class="panel-body">
                ...
                <div class="filter mb-xlg" style="padding: 15px; border: 1px solid #444;">
                    ...
                    <form class="form-horizontal" action="{{ url('admin/transaction/search/') }}" enctype="multipart/form-data" method="post" id="searchForm">

                        <input type="hidden" value="{{ json_encode($merchants) }}" id="merchants" />
                        <input type="hidden" value="{{ json_encode($shops) }}" id="shops" />
                        <input type="hidden" value="{{ json_encode($list) }}" id="list" />
                        <input type="hidden" value="{{ json_encode($paycycles) }}" id="paycycles" />
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-2">
                                <input id="transaction_id" name="transaction_id" class="form-control" value="{{ $transaction_id }}" />
                            </div>

                            <label class="col-sm-1 control-label" style="width: 70px;">ID</label>
                            <div class="col-sm-2">
                                <input id="t_id" name="t_id" type="text" class="form-control" value="{{ $t_id }}" />
                            </div>
                        ...
                        <div class="row mt-sm">
                                ...
                                <div class="row">
                                    <div class="col-sm-12">
                                        <select id="status" name="status"  class="form-control">
                                            ...
                                            <option value="失敗" @if($status=='失敗')selected @endif>失敗</option>
                                            <option value="未決済" @if($status=='未決済')selected @endif>未決済</option>
                                            <option value="返金申請" @if($status=='返金申請')selected @endif>返金申請</option>
                                            <option value="返金完了" @if($status=='返金完了')selected @endif>返金完了</option>
                                            <option value="返金失敗" @if($status=='返金失敗')selected @endif>返金失敗</option>
                                            <option value="CB調整中" @if($status=='CB調整中')selected @endif>CB調整中</option>
                                            <option value="CB確定" @if($status=='CB確定')selected @endif>CB確定</option>
                                            <option value="テスト" @if($status=='テスト')selected @endif>テスト</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="col-sm-2">
                                <div class="row">
                                    <div class="col-sm-12">
                                        <input id="agency_name" name="agency_name" type="text" class="form-control" value="{{ $agency_name }}" />
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row mt-sm">
                            ...
                            <div class="col-sm-2">
                                <div class="row">
                                    <div class="col-sm-12">
                                        <input id="card_number" name="card_number" type="number" class="form-control" value="{{ $card_number }}" />
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="col-sm-2">
                                <div class="row">
                                    <div class="col-sm-12">
                                        <input id="card_holder" name="card_holder" type="text" class="form-control" value="{{ $card_holder }}" />
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="col-sm-2">
                                <div class="row">
                                    <div class="col-sm-12">
                                        <input id="tel" name="tel" type="number" class="form-control" value="{{ $tel }}" />
                                    </div>
                                </div>
                            </div>
                    ...
                    <table class="table table-bordered table-striped text-center">
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td>{{ number_format($visa->t_count) }}</td>
                                <td class="green">¥{{ number_format(ceil($visa->t_amount)) }}</td>
                                <td>{{ number_format($visa->ss_count) }}</td>
                                <td class="green">¥{{ number_format(ceil($visa->ss_amount)) }}</td>
                                <td>{{ number_format($visa->ff_count) }}</td>
                                <td class="green">¥{{ number_format(ceil($visa->ff_amount)) }}</td>
                                <td>{{ number_format($visa->cc_count) }}</td>
                                <td class="green">¥{{ number_format(ceil($visa->cc_amount)) }}</td>
                                <td>{{ number_format($visa->cb_count) }}</td>
                                <td class="green">¥{{ number_format(ceil($visa->cb_amount)) }}</td>
                                <td>{{ number_format($visa->sms_count) }}</td>
                                <td class="green">¥{{ number_format(ceil($visa->sms_amount)) }}</td>
                                <td>{{ number_format($visa->sms_ss_count) }}</td>
                                <td>{{ number_format($visa->banner_count) }}</td>
                                <td>{{ number_format($visa->qr_count) }}</td>
                            </tr>
                            <tr>
                                <td><strong>MASTER</strong></td>
                                <td>{{ number_format($master->t_count) }}</td>
                                <td class="green">¥{{ number_format(ceil($master->t_amount)) }}</td>
                                <td>{{ number_format($master->ss_count) }}</td>
                                <td class="green">¥{{ number_format(ceil($master->ss_amount)) }}</td>
                                <td>{{ number_format($master->ff_count) }}</td>
                                <td class="green">¥{{ number_format(ceil($master->ff_amount)) }}</td>
                                <td>{{ number_format($master->cc_count) }}</td>
                                <td class="green">¥{{ number_format(ceil($master->cc_amount)) }}</td>
                                <td>{{ number_format($master->cb_count) }}</td>
                                <td class="green">¥{{ number_format(ceil($master->cb_amount)) }}</td>
                                <td>{{ number_format($master->sms_count) }}</td>
                                <td class="green">¥{{ number_format(ceil($master->sms_amount)) }}</td>
                                <td>{{ number_format($master->sms_ss_count) }}</td>
                                <td>{{ number_format($master->banner_count) }}</td>
                                <td>{{ number_format($master->qr_count) }}</td>
                            </tr>
                            <tr>
                                <td><strong>JCB</strong></td>
                                <td>{{ number_format($jcb->t_count) }}</td>
                                <td class="green">¥{{ number_format(ceil($jcb->t_amount)) }}</td>
                                <td>{{ number_format($jcb->ss_count) }}</td>
                                <td class="green">¥{{ number_format(ceil($jcb->ss_amount)) }}</td>
                                <td>{{ number_format($jcb->ff_count) }}</td>
                                <td class="green">¥{{ number_format(ceil($jcb->ff_amount)) }}</td>
                                <td>{{ number_format($jcb->cc_count) }}</td>
                                <td class="green">¥{{ number_format(ceil($jcb->cc_amount)) }}</td>
                                <td>{{ number_format($jcb->cb_count) }}</td>
                                <td class="green">¥{{ number_format(ceil($jcb->cb_amount)) }}</td>
                                <td>{{ number_format($jcb->sms_count) }}</td>
                                <td class="green">¥{{ number_format(ceil($jcb->sms_amount)) }}</td>
                                <td>{{ number_format($jcb->sms_ss_count) }}</td>
                                <td>{{ number_format($jcb->banner_count) }}</td>
                                <td>{{ number_format($jcb->qr_count) }}</td>
                            </tr>
                            <tr>
                                <td><strong>AMEX</strong></td>
                                <td>{{ number_format($amex->t_count) }}</td>
                                <td class="green">¥{{ number_format(ceil($amex->t_amount)) }}</td>
                                <td>{{ number_format($amex->ss_count) }}</td>
                                <td class="green">¥{{ number_format(ceil($amex->ss_amount)) }}</td>
                                <td>{{ number_format($amex->ff_count) }}</td>
                                <td class="green">¥{{ number_format(ceil($amex->ff_amount)) }}</td>
                                <td>{{ number_format($amex->cc_count) }}</td>
                                <td class="green">¥{{ number_format(ceil($amex->cc_amount)) }}</td>
                                <td>{{ number_format($amex->cb_count) }}</td>
                                <td class="green">¥{{ number_format(ceil($amex->cb_amount)) }}</td>
                                <td>{{ number_format($amex->sms_count) }}</td>
                                <td class="green">¥{{ number_format(ceil($amex->sms_amount)) }}</td>
                                <td>{{ number_format($amex->sms_ss_count) }}</td>
                                <td>{{ number_format($amex->banner_count) }}</td>
                                <td>{{ number_format($amex->qr_count) }}</td>
                            </tr>
                            <tr>
                                <td><strong>合計</strong></td>
                                <td>{{ number_format($visa->t_count+$master->t_count+$jcb->t_count+$amex->t_count) }}</td>
                                <td class="green">¥{{ number_format(ceil($visa->t_amount+$master->t_amount+$jcb->t_amount+$amex->t_amount)) }}</td>
                                <td>{{ number_format($visa->ss_count+$master->ss_count+$jcb->ss_count+$amex->ss_count) }}</td>
                                <td class="green">¥{{ number_format(ceil($visa->ss_amount+$master->ss_amount+$jcb->ss_amount+$amex->ss_amount)) }}</td>
                                <td>{{ number_format($visa->ff_count+$master->ff_count+$jcb->ff_count+$amex->ff_count) }}</td>
                                <td class="green">¥{{ number_format(ceil($visa->ff_amount+$master->ff_amount+$jcb->ff_amount+$amex->ff_amount)) }}</td>
                                <td>{{ number_format($visa->cc_count+$master->cc_count+$jcb->cc_count+$amex->cc_count) }}</td>
                                <td class="green">¥{{ number_format(ceil($visa->cc_amount+$master->cc_amount+$jcb->cc_amount+$amex->cc_amount)) }}</td>
                                <td>{{ number_format($visa->cb_count+$master->cb_count+$jcb->cb_count+$amex->cb_count) }}</td>
                                <td class="green">¥{{ number_format(ceil($visa->cb_amount+$master->cb_amount+$jcb->cb_amount+$amex->cb_amount)) }}</td>
                                <td>{{ number_format($visa->sms_count+$jcb->sms_count+$master->sms_count+$amex->sms_count) }}</td>
                                <td class="green">¥{{ number_format(ceil($visa->sms_amount+$master->sms_amount+$jcb->sms_amount+$amex->sms_amount)) }}</td>
                                <td>{{ number_format($visa->sms_ss_count+$master->sms_ss_count+$jcb->sms_ss_count+$amex->sms_ss_count) }}</td>
                                <td>{{ number_format($visa->banner_count+$master->banner_count+$jcb->banner_count+$amex->banner_count) }}</td>
                                <td>{{ number_format($visa->qr_count+$master->qr_count+$jcb->qr_count+$amex->qr_count) }}</td>
                            </tr>
                ...
                <table class="table table-bordered table-striped text-center" id="transaction_datatable">
                    ...
                    @foreach($list as $transaction)
                        ...
                        @if($transaction->parent == 0)
                            <tr>
                        @else
                            @if(($transaction->status == '返金完了') || ($transaction->status == 'CB確定'))
                        ...
                        @endif
                            ...
                            @elseif(($transaction->status == '返金完了') || ($transaction->status == 'CB確定'))
                            ...
                            @endif
                                ...
                                @if($transaction_id != '')
                                    @if((($transaction->child == 0) && ($transaction->parent != 0)) || (($transaction->child == 0) && ($transaction->parent == 0)))
                                        <select style="background-color: transparent; border: none; text-align-last: center;" onchange="changeTransaction(this, '{{ $transaction->id }}', '{{ $transaction->memo }}', '{{ $transaction->status }}')">
                                            <option value="成功" style="background-color: #00b0f0; color: white" @if($transaction->status == '成功') selected @endif>成功</option>
                                            <option value="失敗" style="background-color: #fff2cc" @if($transaction->status == '失敗') selected @endif>失敗</option>
                                            <option value="未決済" style="background-color: #fff2cc" @if($transaction->status == '未決済') selected @endif>未決済</option>
                                            <option value="返金申請" style="background-color: #fff2cc" @if($transaction->status == '返金申請') selected @endif>返金申請</option>
                                            <option value="返金完了" style="background-color: #ffc000" @if($transaction->status == '返金完了') selected @endif>返金完了</option>
                                            <option value="返金失敗" style="background-color: #fff2cc" @if($transaction->status == '返金失敗') selected @endif>返金失敗</option>
                                            <option value="CB調整中" style="background-color: #fff2cc" @if($transaction->status == 'CB調整中') selected @endif>CB調整中</option>
                                            <option value="CB確定" style="background-color: #ffc000" @if($transaction->status == 'CB確定') selected @endif>CB確定</option>
                                            @if(($transaction->child == 0) && ($transaction->parent != 0))
                                                <option value="削除" style="background-color: red; color: white;" @if($transaction->status == '削除') selected @endif>削除</option>
                                            @endif
                                        </select>
                                    @else
                                        @if(($transaction->child != 0) && ($transaction->parent == 0))
                            ...
                            <td class="option hidden">{{ $transaction->cardtype }}</td>
                            <td class="status">{{ $transaction->cardtype }}</td>
                            <td> &zwnj;{{ $transaction->card_number }}</td>
                            <td>&zwnj;{{ $transaction->expiry_date }}</td>
                            <td>&zwnj;{{ $transaction->phone }}</td>
                            <td>{{ $transaction->payment_method }}</td>
                            <td id="merchant_name{{ $transaction->id }}">
                                @if($transaction->merchant)
                                    {{ $transaction->merchant->name }}
                                @endif
                            </td>
                            <td class="option hidden" id="merchant_u_name{{ $transaction->id }}">
                                @if($transaction->merchant)
                                    {{ $transaction->merchant->u_name }}
                                @endif
                            </td>
                            <td class="option hidden" id="merchant_u_email{{ $transaction->id }}">
                                @if($transaction->merchant)
                                    {{ $transaction->merchant->u_email }}
                                @endif
                            </td>
                            <td id="shop_name{{ $transaction->id }}">
                                @if($transaction->shop && $transaction->userType == '2')
                                    {{ $transaction->shop->name }}
                                @endif
                            </td>
                            <td id="shop_s_name{{ $transaction->id }}">
                                @if($transaction->shop && $transaction->userType == '2')
                                    {{ $transaction->shop->s_name }}
                                @endif
                            </td>
                            <td id="pay_cycle{{ $transaction->id }}">
                                {{ $transaction->pay_cycle }}
                            </td>

                            <td class="option hidden" id="merchant_u_status{{ $transaction->id }}">
                                @if($transaction->userType == '1')
                                    @if($transaction->merchant)
                                        {{ $transaction->merchant->u_status }}
                                    @endif
                                @else
                                    @if($transaction->shop)
                                        {{ $transaction->shop->s_status }}
                                    @endif
                                @endif
                            </td>
                            <td class="option hidden" id="merchant_u_bankcode{{ $transaction->id }}">
                                @if($transaction->merchant)
                                    &zwnj;{{ $transaction->merchant->u_bankcode }}
                                @endif
                            </td>
                            <td class="option hidden" id="merchant_u_branchcode{{ $transaction->id }}">
                                @if($transaction->merchant)    
                                    {{ $transaction->merchant->u_branchcode }}
                                @endif
                            </td>
                            <td class="option hidden" id="merchant_u_holdertype{{ $transaction->id }}">
                                @if($transaction->merchant)
                                    {{ $transaction->merchant->u_holdertype }}
                                @endif
                            </td>
                            <td class="option hidden" id="merchant_u_holdernumber{{ $transaction->id }}">
                                @if($transaction->merchant)
                                    {{ substr($transaction->merchant->u_holdernumber,0,7) }}
                                @endif
                            </td>
                            <td class="option hidden" id="merchant_u_holdername{{ $transaction->id }}">
                                @if($transaction->merchant)
                                    {{ $transaction->merchant->u_holdername_sei . ' ' . $transaction->merchant->u_holdername_mei }}
                                @endif
                            </td>
                            <td class="option hidden" id="shop_s_agency_name{{ $transaction->id }}">
                                @if(($transaction->userType == '2') && ($transaction->shop))
                                    {{ $transaction->shop->s_agency_name }}
                                @endif
                            </td>
                            <td class="option hidden" id="shop_s_agency_fee{{ $transaction->id }}">
                                @if(($transaction->userType == '2') && $transaction->shop)
                                    {{ $transaction->shop->s_agency_fee }}%
                                @endif
                            </td>

                            <td>{{ $transaction->mid }}</td>
                            <td>{{ $transaction->transaction_no }}</td>
                            <td class="option hidden">{{ $transaction->memo }}</td>
                        </tr>
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body p-md">
                            <div class="form-group">
                                ...
                                <input type="hidden" name="pagelen" id="pagelen" value="{{ $pagelen }}" />
                            </div>
                        </div>
...
@section('page_js_vendor')
    ...
    <script src="{{ asset('public/admin/js/views/transaction.js') }}"></script>
    ...
    <script>
        ...
        $( document ).ready(function() {
            ...
            $('select[name="transaction_datatable_length"]').on('change', function() {
                ...
                window.location.href = '{{ url('/admin/transaction/') }}' + '?' + $('#searchForm').serialize();
            });
        });
        ...
        {
            var url = "{{ url('/admin/transaction/delete') }}";
            $.ajax({
                type: 'POST',
                url: url,
